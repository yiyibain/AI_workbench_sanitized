...
const DATA_QUERY_TOOLS = [
    {
        ...
        function: {
            name: 'queryByDosage',
            description: '按剂量筛选数据，分析品牌在不同剂量下的表现差异。可以传入"all"查看所有剂量的对比，也可以传入具体剂量（如"低剂量"、"高剂量"）查看该剂量的详细数据。建议先调用dosage="all"查看整体分布，再针对不同剂量深入分析。',
            parameters: {
                type: 'object',
                properties: {
                    dosage: {
                        type: 'string',
                        description: '剂量名称，如"低剂量"、"高剂量"。如果传入"all"，则返回所有剂量的统计信息（推荐先调用此方式查看整体分布）'
                    },
                    brand: {
                        type: 'string',
                        description: '可选：品牌名称，如"产品1"、"竞品A"。如果不提供，则返回所有品牌的数据。支持模糊匹配，如"产品1"可以匹配"产品1(分子式A钙片)"等'
                    }
                },
                required: ['dosage']
            }
    ...
    //         packageSize: {
    //           type: 'string',
    //           description: '包装大小，如"大包装"、"小包装"、"高剂量大包装规格"、"低剂量小包装规格"等'
    //         },
    //         dosage: {
    //           type: 'string',
    //           description: '剂量，如"低剂量"、"高剂量"'
    //         },
    //         brand: {
    //           type: 'string',
    //           description: '可选：品牌名称，如"产品1"、"竞品A"。支持模糊匹配'
    //         }
    //       }
    //     }
    //   }
        ...
        function: {
            ...
            parameters: {
                ...
                properties: {
                    dosage: {
                        type: 'string',
                        description: '可选：剂量，如"低剂量"、"高剂量"。可以结合brand参数查看特定品牌特定剂量的WD。建议对比同一品牌不同剂量的WD，分析剂量对分销的影响'
                    },
                    brand: {
                        type: 'string',
                        description: '可选：品牌名称，如"产品1"、"竞品A"。支持模糊匹配。如果不提供，则返回所有品牌的WD数据。建议结合dosage参数，对比同一品牌不同剂量的WD'
                    },
                    packageSize: {
                        type: 'string',
                        description: '可选：包装大小，如"大包装"、"小包装"、"高剂量大包装规格"等。可以结合brand和dosage参数进行多维度交叉分析'
                    }
                }
            }
        }
...
// 模糊匹配品牌名称（用于处理"产品1"、"产品1片"、"产品1胶囊"等变体）
// 通用模糊匹配函数（用于所有维度的查询）
function fuzzyMatch(dataValue, targetValue) {
    ...
    if (normalizedTargetValue.includes('mg')) {
        ...
        // 如果数据值包含目标值（如"低剂量"包含在"低剂量小包装规格"中），也匹配
        if (normalizedDataValue.includes(normalizedTargetValue)) {
            return true;
        }
        // 如果目标值包含数据值（如"低剂量小包装规格"包含"低剂量"），也匹配
    ...
    // 对于其他字段，使用原有的模糊匹配逻辑
    // 例如："产品1" 匹配 "产品1(分子式A钙片)"、"浙江" 匹配 "浙江省"等
    if (normalizedDataValue.includes(normalizedTargetValue) ||
        ...
        switch (functionName) {
            ...
            case 'queryWD': {
                ...
                if (wdKey) {
                    ...
                    result.push('数据库中未包含WD字段（请检查目录准入是否为"WD"、"wd"、"分销"、"分销率"或"加权铺货率"）');
                }
                break;
            }
            default:
...
- 整体方法论是将市场切分为数个细分市场，分析其中"剪刀差"（剪刀差的定义见后文）
  - 如，目标治疗领域市场可按照 "分子式×剂量"被切分为多个细分市场，例如"分子式A‑低剂量""分子式A‑高剂量"等，进一步分析各个细分市场内品牌的表现
- 数据维度说明
  - 可用于切分市场的数据维度包括：分子式（如分子式A）、Class（如相关分子式）、原研/仿制、品牌最早上市时间、品牌平均单片价格、品牌平均单盒价格、品牌平均包装大小、是否中标品、品牌在零售渠道内份额、省份
...
重点关注的对象仍然是某医药公司品牌表现（如${selectedBrand}），可以重点关注对比竞品品牌的"剪刀差"（如竞品A）

2. 剪刀差分类
- 同品牌，不同细分市场剪刀差：同一品牌在不同细分市场中的表现分化明显，如${selectedBrand}在低剂量 与高剂量这两个市场内的份额和增长率差异显著；或${selectedBrand} 低剂量 在浙江vs 全国平均表现差异大
- 同细分市场，不同品牌剪刀差：同一细分市场内，不同品牌的份额水平或走势出现"此消彼长"，如在"分子式A‑低剂量"细分市场中，${selectedBrand}份额持续下降，而核心竞争品牌份额持续上升
- 同细分市场，不同价格 / 规格剪刀差：同一"分子式 × 剂量"下，不同价格带或规格包型间的表现分化，如高价规格份额稳定提升，而低价规格份额持续流失，或反之
- 同为原研品，不同细分市场内表现剪刀差：同属原研品（如分子式A原研品 – ${selectedBrand}、分子式B原研品 – 竞品A）的两个产品，在分子式内表现差异大，如两个分子式同样在2019年参与集采，${selectedBrand}在阿伐零售渠道内份额从60%降低到30%，竞品A在分子式B零售渠道内份额从60%降低到42%，竞品A份额下降幅度更小
...
- 第二步：针对发现的剪刀差，发现其中重复项目，并合并，具体的判断方法
    - 如果两条剪刀差项目中对比对象（如同为产品1 vs. 竞品A）、渠道（如同为零售渠道）、份额计算口径（包含输出内容中提到的所有份额计算口径，如均包含了分子式内份额）相同，则为同一条剪刀差，应该合并
      - 举例来说：
        - 剪刀差A：
          - 零售渠道分子式内份额落后：产品1相对竞品A弱势
          - 现象：在零售渠道整体目标治疗领域市场中，产品1在其所属的‘分子式A’分子式内份额为29.92%，而核心竞品竞品A在其所属的‘分子式B’分子式内份额为39.11%，产品1在分子式内的统治力落后竞品A近10个百分点。
        - 剪刀差B
          - 同细分市场品牌间此消彼长：分子式A市场内产品1与仿制品份额剪刀差
          - 现象：在‘分子式A’细分市场（占整体市场55.53%）内，原研品牌产品1份额为29.92%，而主要仿制品牌竞品B份额已达21.68%，竞品C为10.84%，竞品D为7.83%。产品1份额虽仍为第一，但与头部仿制品牌的差距正在快速缩小，呈现‘此消彼长’的竞争态势。
        - 这两条剪刀差对比的均为产品1vs竞品A，渠道均为零售渠道，均提及了“品牌在分子式内份额”这一份额计算口径，因此属于重复项目- 第三步：针对特定的剪刀差项目，深挖其背后原因
  - 深挖时，优先使用数据库中已经提供的数据维度进行深挖，比如
    - 省份，是否品牌表现主要受小部分省份拖累？如果是，这些省份的共同点是什么？（比如集采政策严格、集采中分子式A仅有低剂量中标、都属于东部分省份等）
    - 产品特性，是否品牌的特定品规销售表现差（包含价格带、包装大小、剂量维度，比如大包装、低剂量低剂量产品）？如果是，限制这些品规销售的原因是什么？（比如大包装产品渠道分销WD表现不佳）
    - 公域认知度，是否品牌的公域认知度不如竞对，导致份额较低？为何公域认知度较低？
    - 渠道间价差，是否品牌在电商中的价格高于零售，导致患者都去电商购买，而不在零售购买？

以${selectedBrand}-零售市场为例，建议的分析步骤是：
- 第一步：先看${selectedBrand}在零售目标治疗领域整体市场中的表现，再按"分子式 × 剂量→时间序列"逐级下钻，在每个层级将${selectedBrand}与主要竞品及其自身其他细分市场对比，围绕"同品牌内部 / 品牌间 / 渠道间 / 时间趋势 / 价格规格"这几类剪刀差视角系统性地扫描和归纳问题
...
- 具体格式：中文，bullet point形式，每一个"剪刀差"用统一结构描述：
  - 小标题：一句简短的话，先概括问题，再点出维度（例如"零售渠道分子式内份额落后：${selectedBrand}相对竞品A弱势"）。
  - 现象描述：用1–2 句话说明具体数据表现（谁高谁低、差多少、和哪一类对比，例如"医院内与竞品A持平，零售渠道内落后了xx%，其中${selectedBrand}xx%，竞品Axx%"）。**所有分析都要引用真实数据来说明，数据要详细、真实。数据要具体（清晰说明时间框架、增速计算口径 - pdot/销售额）。**
- 输出举例：医院内，${selectedBrand}分子式内份额与竞品A持平（均为~12%），零售渠道内，${selectedBrand}在分子式内份额低于竞品A（~9%对比~12%）；拆分来看，${selectedBrand}主要是低剂量中标省份（如安徽、合肥）的份额明显低。同时，发现${selectedBrand}低剂量中标省份的WD较低（44对比其他省份60），基于WD分销作为零售的重要因素，可能存在进一步提升的空间
...
请以JSON格式输出，包含以下结构：
{
  "scissorsGaps": [
    {
      "title": "简短标题，概括问题和维度",
      "phenomenon": "现象描述：用1-2句话说明具体数据表现（必须引用真实数据，清晰说明时间框架、增速计算口径），例如'医院内与竞品A持平（均为~12%），零售渠道内落后了3个百分点，其中${selectedBrand}9%，竞品A12%；拆分来看，${selectedBrand}主要是低剂量中标省份（如安徽、合肥）的份额明显低，同时发现${selectedBrand}低剂量中标省份的WD较低（44对比其他省份60）'"
    }
  ]
}
    ...
    try {
        ...
        if (!DEEPSEEK_API_KEY) {
            ...
            responseText = JSON.stringify({
                scissorsGaps: [
                    {
                        title: "零售渠道分子式内份额落后：产品1相对竞品A弱势",
                        phenomenon: "医院内产品1分子式内份额与竞品A持平（均为~12%），零售渠道内产品1在分子式内份额低于竞品A（~9%对比~12%，差距3个百分点）；拆分来看，产品1主要是低剂量中标省份（如安徽、合肥）的份额明显低。同时，发现产品1低剂量中标省份的WD较低（44对比其他省份60），基于WD分销作为零售的重要因素，可能存在进一步提升的空间"
...
export async function analyzeProblemsAndStrategies(scissorsGaps, selectedBrand, marketData, availableDimensions, userFeedback, maxProblems = 10, confirmedProblems) {
    ...
    if (confirmedProblems && confirmedProblems.length > 0) {
        const systemPromptForCauses = `你是一名负责零售渠道目标治疗领域市场的资深数据分析专家。
基于已确认的问题列表，请完成成因分析。
...
2. **queryWD**: 查询分销率WD数据，分析渠道铺货情况。**特别重要**：建议查询相同品牌不同剂量的WD，对比分析不同剂量下的分销表现差异（如queryWD({brand: '产品1', dosage: '低剂量'})和queryWD({brand: '产品1', dosage: '高剂量'})）
...
5. **queryWD**: 查询分销率WD数据，分析渠道铺货情况。**特别重要**：建议查询相同品牌不同剂量的WD，对比分析不同剂量下的分销表现差异（如queryWD({brand: '产品1', dosage: '低剂量'})和queryWD({brand: '产品1', dosage: '高剂量'})）
...
1. **省份维度**：是否品牌表现主要受小部分省份拖累？如果是，这些省份的共同点是什么？（比如集采政策严格、集采中分子式A仅有低剂量中标、都属于东部分省份等）
2. **产品特性维度**：是否品牌的特定品规销售表现差（包含价格带、包装大小、剂量维度，比如大包装、低剂量低剂量产品）？如果是，限制这些品规销售的原因是什么？（比如大包装产品渠道分销WD表现不佳）
...
请以JSON格式输出：
{
  "causes": [
    {
      "problem": "问题描述",
      "statement": "总结性的分析陈述，必须包含环境因素、商业推广因素、产品因素、资源分配因素四个维度的分析，但不要按四个因素分类列出，而是自然地融合在一句连贯的statement中。例如：'医院内，产品1分子式内份额与竞品A持平（均为~12%），零售渠道内，产品1在分子式内份额低于竞品A（~9%对比~12%）。从环境因素看，产品1主要是低剂量中标省份（如安徽、合肥）的份额明显低，这些省份集采政策严格，仅低剂量中标导致院内处方向零售转移受限。从产品因素看，产品1在低剂量剂量数据量是高剂量的近2倍，但低剂量的WD仅为25.92，远低于高剂量的58.32，显示产品剂量结构不平衡。从商业推广因素看，产品1低剂量的WD为25.92，而竞品A低剂量的WD为46.85，两者相差20.93个百分点，说明在低剂量剂量的商业推广和渠道覆盖上，产品1明显弱于竞品A。从资源分配因素看，产品1资源分配存在严重不平衡：在低剂量剂量上投入不足（WD仅25.92），而在高剂量上过度投入（WD达58.32），相比之下，竞品A在低剂量剂量上资源投入更充分（WD46.85），这解释了为什么竞品A在分子式内份额更高。'"
    }
  ]
}
...
请严格按照JSON格式输出，只输出JSON，不要包含其他文字说明。注意：只需要输出成因分析（causes），不需要输出策略建议（strategies）。`;
                ...
                if (!DEEPSEEK_API_KEY) {
                    ...
                    responseText = JSON.stringify({
                        causes: [{
                                problem: problem,
                                statement: '基于数据库维度分析：通过省份维度分析发现，品牌表现主要受部分省份拖累，这些省份的共同点是集采政策严格、集采中分子式A仅有低剂量中标。同时，通过产品特性维度分析发现，大包装产品渠道分销WD表现不佳（WD为44，对比其他省份60），导致院外承接院内处方能力差。',
                            }],
                    });
                }
                else {
                ...
                try {
                    ...
                    // 兜底：尽量从非标准JSON中提取statement，避免单题失败导致无结果
                    const statementMatch = responseText.match(/"statement"\s*:\s*"([\s\S]*?)"\s*}/);
                    if (statementMatch && statementMatch[1]) {
                        allCauses.push({
                            problem: problem,
                            statement: statementMatch[1].replace(/\\n/g, '\n').replace(/\\"/g, '"')
                        });
                        console.warn(`⚠️ 使用兜底提取逻辑解析问题 ${i + 1} 的 statement`);
                    }
                }
            }
            return {
...
2. **queryByProductSpec**: 按产品特性筛选（价格带、包装大小、剂量等），分析特定品规的销售表现
   ...
   - 示例：queryByProductSpec({dosage: '低剂量', brand: '${selectedBrand}'}) 查看低剂量产品数据
3. **queryPriceDifference**: 查询渠道间价差（电商vs零售），分析价格差异对销售的影响
...
- 要提出假设并验证（如"假设：产品1在东部省份WD更高 → 验证：通过查询发现..."）
...
**❌ 错误示例（太机械，不要这样做）**：
"产品1的平均WD为46.33，竞品A的平均WD为39.19。产品1的分销率WD略高于竞品A"

**✅ 正确示例（深度分析，必须这样做）**：
"通过queryWD查询发现，产品1的平均WD为46.33，竞品A为39.19。但进一步分析发现：
1. **剂量分化明显**：通过queryByDosage和queryWD交叉分析，产品1低剂量的WD为40-，而高剂量的WD为50+，存在明显的剂量分化
2. **规格差异**：通过queryByProductSpec和queryWD分析，产品1大包装规格（高剂量大包装规格）的WD为50+，而小包装规格（低剂量小包装规格）的WD为40-，这种差异在低剂量剂量更加明显
3. **竞品对比**：竞品A虽然整体WD较低，但在低剂量剂量的WD反而略高于产品1（42 vs 40），说明产品1在低剂量剂量的分销存在明显短板
4. **可能原因**：结合剂量和规格分析，产品1低剂量大包装规格的WD偏低，可能与低剂量患者更偏好小包装、短疗程有关，而产品1在低剂量剂量的大包装铺货不足"
...
5. **必须引用具体的数值和发现**，如"通过queryWD查询发现，产品1低剂量的WD为40，而高剂量的WD为50，说明存在明显的剂量分化"
6. **必须进行深度分析**，说明"为什么"而不仅仅是"是什么"
7. **只输出JSON，不要包含任何其他文字说明**

**⚠️ 禁止使用占位符文本**：
- ❌ 错误示例："environmentFactors": "环境因素分析"
- ✅ 正确示例："environmentFactors": "通过queryWD查询发现，产品1低剂量的WD为40，而高剂量的WD为50，结合queryByDosage分析，说明低剂量剂量在零售渠道的分销存在明显短板，可能与集采政策导致低剂量中标省份的渠道资源配置不足有关"
...
2. **分销率WD分析**：**特别重要**：必须调用 queryWD({brand: '${selectedBrand}', dosage: '低剂量'}) 和 queryWD({brand: '${selectedBrand}', dosage: '高剂量'}) 对比同一品牌不同剂量的WD
...
1. **主动探索数据模式**：
   - 如果初步查询发现"产品1的平均WD为46.33，竞品A为39.19"，不要只停留在"产品1略高"的结论
   - **必须主动调用更多查询函数**，从多个角度深入分析：
     - 调用 queryByDosage({dosage: 'all', brand: '产品1'}) 查看剂量分布，识别哪些剂量拉高了平均值，哪些剂量拖了后腿
     - **必须调用** queryWD({brand: '产品1', dosage: '低剂量'}) 和 queryWD({brand: '产品1', dosage: '高剂量'}) 对比同一品牌不同剂量的WD
     - 结合剂量和品牌：queryWD({dosage: '低剂量', brand: '产品1'}) 和 queryWD({dosage: '高剂量', brand: '产品1'}) 查看特定剂量的表现
...
3. **提出假设并验证**：
   - 基于初步数据提出假设（如"产品1在高剂量WD更高"）
   - 主动调用相关查询函数验证假设（如queryWD({brand: '产品1', dosage: '低剂量'})和queryWD({brand: '产品1', dosage: '高剂量'})）
   - 如果假设不成立，提出新的假设并继续验证
...
- 必须引用查询函数返回的具体数据，并进行深入分析：
  - ❌ **错误示例（太机械）**："产品1的平均WD为46.33，竞品A的平均WD为39.19。产品1的分销率WD略高于竞品A"
  - ✅ **正确示例（深度分析）**："通过queryWD查询发现，产品1的平均WD为46.33，竞品A为39.19。但进一步分析发现：
    1. 剂量分化明显：通过queryByDosage和queryWD交叉分析，产品1低剂量的WD为40-，而高剂量的WD为50+，存在明显的剂量分化
    2. 规格差异：通过queryByProductSpec和queryWD分析，产品1大包装规格（高剂量大包装规格）的WD为50+，而小包装规格（低剂量小包装规格）的WD为40-，这种差异在低剂量剂量更加明显
    3. 竞品对比：竞品A虽然整体WD较低，但在低剂量剂量的WD反而略高于产品1（42 vs 40），说明产品1在低剂量剂量的分销存在明显短板
    4. 可能原因：结合剂量和规格分析，产品1低剂量大包装规格的WD偏低，可能与低剂量患者更偏好小包装、短疗程有关，而产品1在低剂量剂量的大包装铺货不足"
- 要分析数据分布、异常值、模式，而不仅仅是平均值
- 要提出具体的假设并说明验证结果（如"假设：产品1在高剂量WD更高 → 验证：通过queryWD查询发现，产品1高剂量的WD为50+，而低剂量的WD为40-，假设成立"）
...
\`\`\`json
{
  "causes": [
    {
      "problem": "剪刀差标题",
      "statement": "总结性的分析陈述，必须包含环境因素、商业推广因素、产品因素、资源分配因素四个维度的分析，但不要按四个因素分类列出，而是自然地融合在一句连贯的statement中。WD（分销率）数据只在商业推广因素中提及，其他维度要用domain knowledge来丰富分析。⚠️ 重要：不要在statement中提及金额和份额数据。例如：'医院内，产品1分子式内表现与竞品A持平，零售渠道内，产品1表现低于竞品A。从环境因素看，产品1主要是低剂量中标省份（如安徽、合肥）的表现明显低，这些省份集采政策严格，仅低剂量中标导致院内处方向零售转移受限，同时这些省份的零售市场准入门槛较高，对原研产品的接受度相对较低。从产品因素看，产品1在低剂量剂量数据量是高剂量的近2倍，说明低剂量是主要使用剂量，但产品1在低剂量剂量的产品定位和患者教育可能不足，导致患者对低剂量产品的认知和接受度有限，而高剂量产品虽然使用频率低但单次价值高，更容易获得市场关注。从商业推广因素看，产品1低剂量的WD为25.92，而竞品A低剂量的WD为46.85，两者相差20.93个百分点，说明在低剂量剂量的商业推广和渠道覆盖上，产品1明显弱于竞品A，渠道铺货不足限制了产品的可及性。从资源分配因素看，产品1可能将更多资源投入到高剂量产品的学术推广和医生教育上，而低剂量产品的市场教育和患者管理投入相对不足，相比之下，竞品A在低剂量剂量上的市场投入更充分，包括患者教育、渠道建设和医生沟通，这解释了为什么竞品A表现更好。'"
    }
  ]
}
\`\`\`
...
**statement示例格式**：
"医院内，产品1分子式内表现与竞品A持平，零售渠道内，产品1表现低于竞品A。从环境因素看，产品1主要是低剂量中标省份（如安徽、合肥）的表现明显低，这些省份集采政策严格，仅低剂量中标导致院内处方向零售转移受限，同时这些省份的零售市场准入门槛较高，对原研产品的接受度相对较低。从产品因素看，产品1在低剂量剂量数据量是高剂量的近2倍，说明低剂量是主要使用剂量，但产品1在低剂量剂量的产品定位和患者教育可能不足，导致患者对低剂量产品的认知和接受度有限，而高剂量产品虽然使用频率低但单次价值高，更容易获得市场关注。从商业推广因素看，产品1低剂量的WD为25.92，而竞品A低剂量的WD为46.85，两者相差20.93个百分点，说明在低剂量剂量的商业推广和渠道覆盖上，产品1明显弱于竞品A，渠道铺货不足限制了产品的可及性。从资源分配因素看，产品1可能将更多资源投入到高剂量产品的学术推广和医生教育上，而低剂量产品的市场教育和患者管理投入相对不足，相比之下，竞品A在低剂量剂量上的市场投入更充分，包括患者教育、渠道建设和医生沟通，这解释了为什么竞品A表现更好。"
...
3. **必须引用具体的数值**，如"产品1低剂量的WD为40，高剂量的WD为50"，但**不要提及金额和份额数据**
...
\`\`\`json
{
  "causes": [
    {
      "problem": "${gap.title}",
      "statement": "总结性的分析陈述，必须包含环境因素、商业推广因素、产品因素、资源分配因素四个维度的分析，但不要按四个因素分类列出，而是自然地融合在一句连贯的statement中。必须引用具体数据，但⚠️ 重要：不要在statement中提及金额和份额数据。例如：'医院内，产品1分子式内表现与竞品A持平，零售渠道内，产品1表现低于竞品A。从环境因素看，产品1主要是低剂量中标省份（如安徽、合肥）的表现明显低，这些省份集采政策严格，仅低剂量中标导致院内处方向零售转移受限。从产品因素看，产品1在低剂量剂量数据量是高剂量的近2倍，但低剂量的WD仅为25.92，远低于高剂量的58.32，显示产品剂量结构不平衡。从商业推广因素看，产品1低剂量的WD为25.92，而竞品A低剂量的WD为46.85，两者相差20.93个百分点，说明在低剂量剂量的商业推广和渠道覆盖上，产品1明显弱于竞品A。从资源分配因素看，产品1资源分配存在严重不平衡：在低剂量剂量上投入不足（WD仅25.92），而在高剂量上过度投入（WD达58.32），相比之下，竞品A在低剂量剂量上资源投入更充分（WD46.85），这解释了为什么竞品A表现更好。'"
    }
  ]
}
\`\`\`
...
**要求**：
1. 必须基于上述查询结果进行深度分析
2. 必须引用查询结果中的具体数据（如"产品1低剂量的WD为40"），但**不要提及金额和份额数据**
3. 不能使用占位符文本
